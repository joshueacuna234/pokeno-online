...
// Local Piper TTS for all spoken audio: cards, names, BINGO announcements and test audio.
// No OpenAI API, no paid TTS service, and no large bundle of prerecorded speech files.
const ttsAudioCache=new Map();
const ttsAudioInFlight=new Map();
const PIPER_BIN=process.env.PIPER_BIN||"/opt/piper/piper/piper";
const PIPER_MODEL=process.env.PIPER_MODEL||"/opt/piper/voice/es_MX-claude-high.onnx";
...
function ttsCacheKey(text){return safeSpeechText(text).toLocaleLowerCase("es");}
...
function runPiper(text,outputFile){
  return new Promise((resolve,reject)=>{
    ...
    child.on("close",code=>{
      clearTimeout(timer);
      if(code===0)resolve();
      else reject(new Error(`Piper terminó con código ${code}: ${err.slice(0,300)}`));
    });
    child.stdin.end(text+"\n");
  });
}
async function generateSpeechAudio(text){
  const clean=safeSpeechText(text),key=ttsCacheKey(clean);
  if(!clean)throw new Error("Texto vacío");
  if(ttsAudioCache.has(key))return ttsAudioCache.get(key);
  if(ttsAudioInFlight.has(key))return ttsAudioInFlight.get(key);

  const task=(async()=>{
    const tmp=path.join(os.tmpdir(),`pokeno-tts-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}.wav`);
    try{
      await runPiper(clean,tmp);
      const buffer=await fs.promises.readFile(tmp);
      // Bounded memory cache: enough for the whole 52-card deck plus names/events.
      if(ttsAudioCache.size>=180)ttsAudioCache.delete(ttsAudioCache.keys().next().value);
      ttsAudioCache.set(key,buffer);
      return buffer;
    }finally{
      fs.promises.unlink(tmp).catch(()=>{});
    }
  })().finally(()=>ttsAudioInFlight.delete(key));

  ttsAudioInFlight.set(key,task);
  return task;
}
function prewarmSpeech(text){
  generateSpeechAudio(text).catch(err=>console.warn("No se pudo precargar Piper:",err.message));
}

// General endpoint used by the browser as a normal audio source.
app.get("/api/tts",async(req,res)=>{
  const text=safeSpeechText(req.query.text);
...
app.get("/api/tts-status",(_,res)=>{
  res.json({
    ...
    cachedClips:ttsAudioCache.size
  });
});
...
function createSessionObject(hostName,config){
  ...
  return {
    ...
    hosts:new Map(),disconnectedHosts:new Map(),hostConnected:true,hostTimer:null,status:"lobby",countdownEndsAt:null,
...
function registerHost(s,socket,name,kind,reconnectToken){
  const rt=reconnectToken||token();
  s.hosts.set(socket.id,{socketId:socket.id,name:String(name||"Host").trim().slice(0,30),kind,reconnectToken:rt});
  s.disconnectedHosts.delete(rt);
  if(s.hostTimer){clearTimeout(s.hostTimer);s.hostTimer=null;}
  s.hostConnected=true;
  socket.join(s.code);
  socket.data.code=s.code;
  socket.data.role="host";
  socket.data.hostKind=kind;
  socket.data.hostReconnectToken=rt;
  return rt;
}
...
io.on("connection",socket=>{
  socket.on("createSession",async({hostName,config,origin},cb)=>{
    ...
    registerHost(s,socket,hostName||"Host","primary",s.hostToken);prewarmSpeech(hostName||"Host");
  ...
  socket.on("createNewSession",async({config,origin},cb)=>{
    ...
    registerHost(s,socket,hostName||"Host","primary",s.hostToken);
    socket.data.hostToken=s.hostToken;
    const qr=await roomQr(origin,s.code);
    cb?.({ok:true,hostToken:s.hostToken,cohostCode:s.cohostCode,...qr,state:publicState(s)});emitState(s);
  });
  ...
  socket.on("joinCohost",({hostCode,name},cb)=>{
    ...
    const reconnectToken=registerHost(s,socket,name||"Segundo host","secondary");prewarmSpeech(name||"Segundo host");
    cb?.({ok:true,role:"host",hostKind:"secondary",hostReconnectToken:reconnectToken,cohostCode:s.cohostCode,state:publicState(s)});
    emitState(s);
  });

  socket.on("reconnectHost",({code,reconnectToken},cb)=>{
    const s=sessions.get(String(code||"").trim().toUpperCase());
    const rt=String(reconnectToken||"");
    if(!s||!rt)return cb?.({ok:false});
    let h=s.disconnectedHosts.get(rt);
    if(!h && rt===s.hostToken)h={name:s.hostName,kind:"primary"};
    if(!h)return cb?.({ok:false});
    registerHost(s,socket,h.name,h.kind,rt);
    socket.data.hostToken=h.kind==="primary"?s.hostToken:undefined;
    cb?.({ok:true,role:"host",hostKind:h.kind,hostReconnectToken:rt,cohostCode:s.cohostCode,state:publicState(s)});
    emitState(s);
  });
  ...
  socket.on("joinSession",({code,name,avatar,password,role},cb)=>{
    ...
    if(joinRole==="spectator"){
      ...
      s.spectators.set(pt,sp);prewarmSpeech(sp.name);socket.join(code);socket.data.code=code;socket.data.role="spectator";socket.data.spectatorToken=pt;
    ...
    s.players.set(pt,p);prewarmSpeech(p.name);socket.join(code);socket.data.code=code;socket.data.role="player";socket.data.playerToken=pt;
  ...
  socket.on("disconnect",()=>{
    ...
    if(socket.data.role==="host"){
      const h=s.hosts.get(socket.id);
      s.hosts.delete(socket.id);
      if(h?.reconnectToken){
        s.disconnectedHosts.set(h.reconnectToken,{name:h.name,kind:h.kind,expiresAt:Date.now()+HOST_GRACE_MS});
      }
      s.hostConnected=s.hosts.size>0;
      if(s.hosts.size===0){
        // Preserve the room for 5 minutes. Pause autoplay so cards are never drawn without a host.
        stopAutoplay(s);
        emitState(s);
        if(s.hostTimer)clearTimeout(s.hostTimer);
        s.hostTimer=setTimeout(()=>{
          if(s.hosts.size===0 && sessions.get(s.code)===s){
            io.to(s.code).emit("sessionClosed");
            sessions.delete(s.code);
          }
        },HOST_GRACE_MS);
...
server.listen(PORT,()=>console.log(`POKENO v4.8.3 en puerto ${PORT}`));